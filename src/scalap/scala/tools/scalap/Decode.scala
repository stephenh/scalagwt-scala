...
object Decode {
  ...
  def scalaSigAnnotationBytes(name: String): Option[Array[Byte]] =
    scalaSigAnnotationBytes(name, getSystemLoader())
    
  def scalaSigAnnotationBytes(name: String, classLoader: ScalaClassLoader): Option[Array[Byte]] =
    scalaSigAnnotationBytes(classLoader.getBytesForClass(name))
  
  def scalaSigAnnotationBytes(bytes: Array[Byte]): Option[Array[Byte]] = {
